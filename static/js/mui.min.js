		...
		handleEvent: function(e) {
			switch (e.type) {
				...
				case 'webkitTransitionEnd': //有个bug需要处理，需要考虑假设没有触发webkitTransitionEnd的情况
					...
					break;
				case 'drag':
                    //临时处理，当有mui-no-draggable时禁止滑动打开菜单
                    if(this.wrapper.querySelector('.mui-no-draggable'))
                        return ;
					var detail = e.detail;
					if (!this.startX) {
...
(function($, window, document, name) {
	...
	var removeBackdrop = function(popover) {
		...
		removeBackdropTimer = $.later(function() {
			if (!popover.classList.contains(CLASS_ACTIVE) && backdrop.parentNode && (backdrop.parentNode === document.body||backdrop.parentNode == document.querySelector('.mui-content'))) {
				if(backdrop.parentNode === document.body)
					document.body.removeChild(backdrop);
				if(backdrop.parentNode == document.querySelector('.mui-content'))
					document.querySelector('.mui-content').removeChild(backdrop);
			}
		}, 350);
	};
	window.addEventListener('tap', function(e) {
	...
	var togglePopover = function(popover, anchor, state) {
		...
		document.querySelector('.mui-content').appendChild(backdrop);
